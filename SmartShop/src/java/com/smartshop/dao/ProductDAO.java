...
public class ProductDAO {
    ...
    private Product map(ResultSet rs) throws SQLException {
        ...
        try {
            p.setPromoEndAt(rs.getTimestamp("promo_end_at"));
        } catch (SQLException ignore) {
        }
    ...
    public Product find(int id) {
        try (Connection c = DB.getConnection(); PreparedStatement ps = c.prepareStatement("SELECT * FROM Products WHERE id=?")) {
            ps.setInt(1, id);
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    return map(rs);
    ...
    public void delete(int id) {
        try (Connection c = DB.getConnection(); PreparedStatement ps = c.prepareStatement("DELETE FROM Products WHERE id=?")) {
            ps.setInt(1, id);
            ps.executeUpdate();
        } catch (SQLException e) {
            throw new RuntimeException(e);
    ...
    public List<Product> top(String type, int limit) {
        ...
        switch (type == null ? "" : type.toLowerCase()) {
            case "featured":
                order = "rating DESC, sold DESC, updated_at DESC";
                break;
            case "new":
            case "newest":
